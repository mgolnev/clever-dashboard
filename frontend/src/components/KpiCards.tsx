...
const METRICS: StageMetricDef[] = [
  { label: "Выручка", additive: true, redemptionOutcome: true, pick: (s) => s.revenue, fmt: (s) => rub(s.revenue), fmtAbs: rubAbs },
  { label: "Заказы", additive: true, redemptionOutcome: true, pick: (s) => s.orders, fmt: (s) => num(s.orders), fmtAbs: numAbs },
...
export default function KpiCards({ current, prev, showCompare = true }: Props) {
  const rates = [
    ...
    { label: "G2N", value: pct(current.g2n), cur: current.g2n, prv: prev.g2n, hint: "чистый выкуп / оформл.", fmtAbs: ppAbs },
    { label: "Выкупаемость", value: pct(current.redemptionRate), cur: current.redemptionRate, prv: prev.redemptionRate, hint: `чистыми из ${num(current.terminal)}`, fmtAbs: ppAbs },
