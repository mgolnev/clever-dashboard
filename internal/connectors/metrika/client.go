// Package metrika получает обезличенные дневные агрегаты сайта через
// Reporting API Яндекс Метрики.
package metrika
...
func New(counterID, token, timezone string) *Client {
	return &Client{
...
func (c *Client) Name() string     { return "metrika" }
func (c *Client) Channel() string  { return "site" }
func (c *Client) Configured() bool { return c.counterID != "" && c.token != "" }
...
func (c *Client) Fetch(ctx context.Context, from, to time.Time) ([]model.DailyTraffic, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("запрос Метрики: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, fmt.Errorf("Метрика HTTP %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
	var payload reportResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, fmt.Errorf("ответ Метрики: %w", err)
	}
